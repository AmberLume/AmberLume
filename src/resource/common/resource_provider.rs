use crate::resource::common::internal_state::InternalState;
use crate::resource::common::res_ref::{ResRef, ResState};
    ...
    fn handle_new(&self, resref: &ResRef<B::Config>) -> ResourceId {
        ...
        if let Some(task_id) = task_id_optional {
            ...
            let handle = Builder::new()
                .name(format!("resource-{}-{}", id, task_id))
                .spawn(move || {
                ...
                .expect("Can't spawn new resource thread");
