...
use crate::compute_pipeline_config::ComputePipelineConfig;
use resource_residency::ResourceBackend;
use index_allocator::ResourceId;
use anyhow::Result;
use ash::vk::{
    ComputePipelineCreateInfo, Pipeline, PipelineCache, PipelineShaderStageCreateInfo,
    ShaderModule, ShaderModuleCreateInfo, ShaderStageFlags, SpecializationInfo,
...
impl ComputePipelineBackend {
    ...
    fn create_shader_module(&self, label: &str, spv: &[u32]) -> Result<ShaderModule> {
        ...
        self.debug_utils.label(shader_module, &format!("shader_module_{}", label));
...
impl ResourceBackend for ComputePipelineBackend {
    ...
    fn create(&self, _id: &ResourceId, config: Self::Config) -> Result<Self::Output> {
        ...
        let spv = self.resource_reader.get_resource(config.shader_name.key())?;
        ...
        let specialization_entries: Vec<SpecializationMapEntry> = config
        ...
        if !config.specialization_entries.is_empty() {
            shader_stage_create_info = shader_stage_create_info.specialization_info(&specialization_info);
        }
