...
void main() {
    ...
    EntityBuffer entity_buffer = EntityBuffer(push_constants.entity_buffer_device_address);
    ...
    if (own_entity != ENTITY_NONE && entity_buffer.data[own_entity].outline.a > 0.0) {
        return;
    }
    ...
    for (int y = -radius; y <= radius; y++) {
        for (int x = -radius; x <= radius; x++) {
            ...
            if (entity_buffer.data[entity_index].outline.a <= 0.0) {
                continue;
            }
    ...
    vec4 color = entity_buffer.data[nearest_entity].outline;
