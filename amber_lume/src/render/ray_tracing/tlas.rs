...
use ash::vk::{
    ...
    DeviceAddress, DeviceOrHostAddressConstKHR, DeviceSize, GeometryTypeKHR,
};
...
impl TLAS {
    pub fn new(
        ...
        let max_instances = limits.resource_limits.max_draw_calls;
    ...
    pub fn next_build_mode(&self, instance_count: u32) -> BuildAccelerationStructureModeKHR {
        ...
        if rebuild {
            self.last_built_count.store(instance_count, Ordering::Relaxed);
...
pub fn instances_geometry(data_address: DeviceAddress) -> AccelerationStructureGeometryKHR<'static> {
